...
public static class StatTable
{
    public static TableLayoutPanel CreateStatPanel(Game model)
    {
        ...
        table.RowStyles.Add(new RowStyle(SizeType.Percent, 40));
        table.Controls.Add(CurrentPlayerLabel(model), 0, 0);
        table.Controls.Add(CreatePassButton(model), 0, 1);
    ...
    private static Label CurrentPlayerLabel(Game model)
    {
        var currentPlayerLabel = new Label
        {
            Dock = DockStyle.Fill,
            Font = new Font("Tahoma", 30, FontStyle.Regular),
            Text = "Player Black current. Want to pass?",
            TextAlign = ContentAlignment.MiddleCenter
        };
        model.CurrentPlayerChanged += () => currentPlayerLabel.Text 
